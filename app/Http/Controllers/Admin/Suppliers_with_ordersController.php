...
$OldBatchExsists = get_cols_where_row(new Inv_itemcard_batches(), array("quantity", "id", "unit_cost_price"), $dataInsertBatch);
    ...
    $OldBatchExsists = get_cols_where_row(new Inv_itemcard_batches(), array("quantity", "id", "unit_cost_price","auto_serial"), array("item_code"=>$info->item_code,"com_code"=>$com_code));
...
$flag=update(new Inv_itemcard_batches(), $dataUpdateOldBatch, array("id" => $OldBatchExsists['id'], "com_code" => $com_code));
if($flag){
    $dataToUpdateItemDetails['batch_auto_serial']=$OldBatchExsists['auto_serial'];
    update(new Suppliers_with_orders_details(),$dataToUpdateItemDetails,array('id'=>$info->id));
}
} else {
...
if($flag){
    $dataToUpdateItemDetails['batch_auto_serial']=$dataInsertBatch['auto_serial'];
    update(new Suppliers_with_orders_details(),$dataToUpdateItemDetails,array('id'=>$info->id));
}
}
